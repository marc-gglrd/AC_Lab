function [agt,ex_done,ap_done]=cr_decide(agt,lab,cf2_on,need_cf2,cf3_on,need_cf3,cf4_on,need_cf4)

%CAREGIVER function to take action
%agt: caregiver object
%lab: lab object
%cf2_on: true when cf2 is on
%need_cf2: true when need to give cf2
%cf3_on: true when cf3 is on
%need_cf3: true when need to give cf3
%explore:  true if exploration done, 0 otherwise
%approach: true if approach    done, 0 otherwise
% ----------------------------------
...
%         %[agt,ndx_xplrd] = cr_explore(agt,lab); %EXPLORE
%         ex_done = true;
%     end
% else
%     if (cf2_on && (ed > (ed_tlrnc * target_ed))) ||...
%        (cf3_on && (pd > (pd_tlrnc * target_pd)))
%         %agt = cr_approach(agt,lab);            %APPROACH
%         ap_done = true;
%     end
% end
%--------------------------------------------------------------------
